---
title: "02-deseq2-sextissuespecific"
output: md_document
---
...
Now, I'll do RNAseq on all the data from the parental care characterization study for each tissue and sex. 

```{r}
colData <- read.csv("../results/00-colData-characterization.csv", header = T, row.names = 1)
countData <- read.csv("../results/00-countData-characterization.csv", header = T, row.names = 1)
```
...
mygroups <- levels(colData$sextissue)

for(i in mygroups){
  print(i)
  colData <- colData %>%
    dplyr::filter(grepl(i, sextissue)) %>%
    droplevels()
  row.names(colData_characterization) <- colData_characterization$V1
  colData <- 
  print(head(colData_characterization))
}
