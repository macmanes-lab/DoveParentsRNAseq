...
Aim I lays the necessary groundwork for establishing a pattern of correlative relationships between observed changes in gene expression and patterns of parental behavior. With this pattern established, we can proceed with experimental manipulations proposed under Aim III to determine causation.
...
This ecludian clustering shows many clusters are heterogenous, consisting of sames from all tissues types, both sexes, and from multiple treatment groups. 
...
subsetWGCNA(whichgroups = c("female.gonad", "female.hypothalamus", "female.pituitary",
            ...
            mytitle = "Characterization dendrogram of samples")
```
...
The rest of the analyses are peformed on each sex and tissue group separately, a 2 x 3 design.  

```{r, message=FALSE, warning=FALSE, cache=TRUE, echo = F, fig.width= 5}
library(cowplot)
ggdraw() + draw_image("../figures/sexes/characterization-2.png") 
```
...
```{r, message=FALSE, warning=FALSE, cache=TRUE, echo = F, fig.width= 5}
a <- ggdraw() + draw_image("../figures/characterization/pca-2.png") 
b <- ggdraw() + draw_image("../figures/characterization/pca-5.png") 
...
![](../figures/characterization/musicalgenes-05.png)
...
**Fig S1a. Female characterization dendrogram with trait heatmap.**. 
...
**Fig S1b. Male characterization dendrogram with trait heatmap.**. 
