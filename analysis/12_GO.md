Exploring relationships between genes associated with mating behavior, lactation, parental care, and endocrine process in the hypothalamic-pituitary-gonad axis of reproction
=============================================================================================================================================================================

1.  Read in list of genes for GO terms: mating beahvior, lactating,
    parental care, and endocrine process.
2.  Read in lists of differentially expressed genes for each transition
    (e.g. incubation day 17 to hatch) in each tissue (e.g. pituitary)
3.  Join the two data frames
4.  Print list of differentially expressed genes that correspond to each
    GO term, separated by either tissue or paretnal stage
5.  Repeat the process for a list of candidate genes instead of GO
    terms.

Made with these R packages
--------------------------

    library(tidyverse)

    ## ── Attaching packages ────────────────────────────────────────────────────────── tidyverse 1.3.0 ──

    ## ✓ ggplot2 3.3.0.9000     ✓ purrr   0.3.3     
    ## ✓ tibble  2.1.3          ✓ dplyr   0.8.3     
    ## ✓ tidyr   1.0.0          ✓ stringr 1.4.0     
    ## ✓ readr   1.3.1          ✓ forcats 0.4.0

    ## ── Conflicts ───────────────────────────────────────────────────────────── tidyverse_conflicts() ──
    ## x dplyr::filter() masks stats::filter()
    ## x dplyr::lag()    masks stats::lag()

    library(knitr)
    library(kableExtra)

    ## 
    ## Attaching package: 'kableExtra'

    ## The following object is masked from 'package:dplyr':
    ## 
    ##     group_rows

    knitr::opts_chunk$set(echo = TRUE, message = F, warning = F)

GO terms
--------

    GO_path <- "../results/goterms/"   # path to the data
    GO_files <- dir(GO_path, pattern = "*.txt") # get file names
    GO_pathfiles <- paste0(GO_path, GO_files)

    allGO <- GO_pathfiles %>%
      setNames(nm = .) %>% 
      map_df(~read_table(.x, col_types = cols(), col_names = FALSE), .id = "file_name") %>% 
      mutate(GO = sapply(strsplit(as.character(file_name),'../results/goterms/'), "[", 2)) %>% 
      mutate(GO = sapply(strsplit(as.character(GO),'.txt'), "[", 1)) %>% 
      mutate(gene = sapply(strsplit(as.character(X1), "[\\\\]|[^[:print:]]" ), "[", 2)) %>% 
      select(GO, gene)  %>%
      filter(gene != "Symbol") %>%
      distinct(GO,gene)  %>%
      mutate(gene = toupper(gene))
    head(allGO)

    ## # A tibble: 6 x 2
    ##   GO               gene         
    ##   <chr>            <chr>        
    ## 1 cellcycleprocess 4930447C04RIK
    ## 2 cellcycleprocess 4930544G11RIK
    ## 3 cellcycleprocess AAAS         
    ## 4 cellcycleprocess ABCB1A       
    ## 5 cellcycleprocess ABRAXAS1     
    ## 6 cellcycleprocess ABRAXAS2

add candidate genes
-------------------

    candidategenesslim <- c("OXT", "AVP", "GNRH1", "GNRHR", 
                        "AR",  "CYP19A1", 
                         "AVPR1A", "AVPR1B", "AVPR2","VIP",
                      "DRD1", "DRD2", 
                      "PRL", "PRLR",  
                        "ESR1","ESR2", "LBH",  
                       
                        "FOS", "JUN", "EGR1", "BDNF"
                          ) 


    candidates <- candidategenesslim

    candidates <- as.data.frame(candidates)
    names(candidates)[1] <- "gene"
    candidates$GO <- "candidategenes"
    candidates <- candidates %>% select(GO, gene)

    allGO <- rbind(candidates, allGO)
    head(allGO)

    ##               GO    gene
    ## 1 candidategenes     OXT
    ## 2 candidategenes     AVP
    ## 3 candidategenes   GNRH1
    ## 4 candidategenes   GNRHR
    ## 5 candidategenes      AR
    ## 6 candidategenes CYP19A1

Differntially expressed genes by stage for each tissue
------------------------------------------------------

    DEG_path <- "../results/DEseq2/"   # path to the data
    DEG_files <- dir(DEG_path, pattern = "*DEGs.csv") # get file names
    DEG_pathfiles <- paste0(DEG_path, DEG_files)

    DEGs <- DEG_pathfiles %>%
      setNames(nm = .) %>% 
      map_df(~read_csv(.x), .id = "file_name") %>%
      mutate(comparison = sapply(strsplit(as.character(file_name),'../results/DEseq2/*'), "[", 2)) %>% 
      mutate(comparison = sapply(strsplit(as.character(comparison),'_DEGs.csv'), "[", 1)) %>% 
      select(gene, comparison)
    head(DEGs)

    ## # A tibble: 6 x 2
    ##   gene         comparison           
    ##   <chr>        <chr>                
    ## 1 LOC107053414 female_gonad_bldg_lay
    ## 2 MUC          female_gonad_bldg_lay
    ## 3 OVSTL        female_gonad_bldg_lay
    ## 4 AOC1         female_gonad_bldg_lay
    ## 5 ETNPPL       female_gonad_bldg_lay
    ## 6 GKN2         female_gonad_bldg_lay

GO and DEGs joined
------------------

    GO_DEGs <- left_join(allGO, DEGs) %>%
      arrange(GO, gene) %>%
      group_by(GO, comparison) %>%
      summarize(genes = str_c(gene, collapse = ", ")) %>% 
      pivot_wider(names_from = comparison, values_from = genes) %>% 
      select(GO, 
             #contains("control_bldg"), 
             contains("bldg_lay"), contains("lay_inc.d3"),
             contains("inc.d3_inc.d9"), contains("inc.d9_inc.d17"), contains("inc.d17_hatch"),
             contains("hatch_n5"), contains("n5_n9")) %>%
      filter(GO %in% c("endocrineprocess", "lactation", 
                       "parentalbehavior", "matingbehavior", "reproduction",
                       "candidategenes", "cellcycleprocess")) %>% 
      map(~.x) %>%
      discard(~all(is.na(.x))) %>%
      map_df(~.x)
    #head(GO_DEGs)

Tissue-specifiic differentially expressed genes
-----------------------------------------------

    fhyp <- GO_DEGs %>% select(GO, starts_with("female_hyp"))
    mhyp <- GO_DEGs %>% select(GO, starts_with("male_hyp"))

    fpit <- GO_DEGs %>% select(GO, starts_with("female_pit"))
    mpit <- GO_DEGs %>% select(GO, starts_with("male_pit"))

    fgon <- GO_DEGs %>% select(GO, starts_with("female_gon"))
    mgon <- GO_DEGs %>% select(GO, starts_with("male_gon"))

    kable(fhyp) 

<table>
<thead>
<tr>
<th style="text-align:left;">
GO
</th>
<th style="text-align:left;">
female\_hypothalamus\_inc.d9\_inc.d17
</th>
<th style="text-align:left;">
female\_hypothalamus\_hatch\_n5
</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left;">
candidategenes
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
BDNF, CYP19A1, DRD1, EGR1
</td>
</tr>
<tr>
<td style="text-align:left;">
cellcycleprocess
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
ACTR1A, AHCTF1, ANAPC5, ANLN, ANP32B, APBB1, ASPM, BECN1, BRCA1, BRINP3,
BUB1B, CAMK2G, CCNB2, CCSAP, CDC25A, CDK5RAP2, CDK5RAP3, CDT1, CENPV,
CEP126, CEP152, CEP192, CEP97, CHMP3, CHMP7, CHORDC1, CKAP2, CKS1B,
CTDP1, DACT1, DCTN1, DCTN2, DDB1, DDIT3, DRD3, E2F1, E2F5, EDN3, EML1,
FAM107A, FAP, FBXL12, FGFR2, GJC2, GPNMB, GRB14, HDAC8, HTT, IGF1, IK,
INCENP, IRF6, IST1, JADE1, KIF20A, KPNB1, LATS1, LLGL2, LMNB1, M1AP,
MAP2K1, MAU2, MCM5, MET, MTBP, MZT1, NCAPH, NEK10, NEUROD1, NFE2L1,
NSFL1C, NUMA1, PCID2, PCNA, PDE4DIP, PDS5A, PEBP1, PHIP, PIN1, PKD2,
PKIA, PKP4, PLCB1, PLK2, PMP22, PPP1R12A, PPP3CA, PRKCE, PSMD13, PSRC1,
RBBP8, RHOB, RHOU, RND2, RPRM, SENP6, SKP2, SLC16A1, SMC5, SMPD3, SNX33,
SPG20, SPIRE2, SYF2, TCF7L2, TDRKH, TEX11, TOM1L1, TOM1L2, TPD52L1,
TXLNG, UBE2C, VCP, WASL, XPO1, ZFYVE19
</td>
</tr>
<tr>
<td style="text-align:left;">
endocrineprocess
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
CRHBP, CRHR2, DRD3, EDN3, FAM129B, FOXL2, KRAS, NPVF, OPRK1, POMC,
RAB11FIP5, RAB8B, RUNX1, TAC1
</td>
</tr>
<tr>
<td style="text-align:left;">
lactation
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
CAV1, ZBTB7B
</td>
</tr>
<tr>
<td style="text-align:left;">
matingbehavior
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
APLP2, DRD1, PGR, THRB
</td>
</tr>
<tr>
<td style="text-align:left;">
parentalbehavior
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
DRD1, GNAQ, OPRK1
</td>
</tr>
<tr>
<td style="text-align:left;">
reproduction
</td>
<td style="text-align:left;">
CFAP44
</td>
<td style="text-align:left;">
ACSBG2, ADCYAP1, ADRA2C, AK7, AKAP9, ALPL, AMHR2, APLP2, ARHGDIB, ASPM,
ATR, BUB1B, CALCA, CCDC39, CCNYL1, CCT3, CD9, CDC25A, CEBPA, CFTR,
CHRNA7, CLDN11, CRHBP, CSF1, CTSB, CYP19A1, DHCR24, DLD, DNAH5, DNAJB6,
DRD1, EGR1, EIF2B2, EIF2B4, ENPP2, ETNK2, FANCL, FGFR2, FOXL2, GAB1,
GGT5, GHSR, GNAQ, GPX4, GRB14, HEY2, HNF4A, HSF2, HSPB11, HTT, IFT88,
IGF1, JAM3, KDM1B, LFNG, LHCGR, LHX8, LHX9, LLGL2, M1AP, MAP2K1, MAP3K4,
MAPK1, MERTK, MET, MFGE8, NCAPH, NCOA3, NEFH, NEK1, NELL2, NPPC, NPY5R,
NR5A1, OCA2, OPRK1, P2RY1, PAFAH1B2, PAIP2, PAQR7, PAQR8, PARK7, PCYT1B,
PEBP1, PGR, PKD2, PLAG1, PLCB1, PMP22, PPARD, PROK2, PSAP, PSMD13,
PSME4, PTK2B, RBBP8, RFX2, RNF114, RUNX1, SCHIP1, SEMA3A, SERPINB5,
SGPL1, SLC22A16, SLCO4C1, SOX17, SOX8, SPATA16, SPATA5, SPEF2, SPIRE2,
STAT3, STK4, SYNE1, TAC1, TACR3, TCF7L2, TDRKH, TEX11, THBD, THRB,
TRPC3, TRPC6, TTC26, VASH2, VMP1, WBP2NL, WDR19, WDR66, WIPF3, WNT2,
WNT3, WNT7A
</td>
</tr>
</tbody>
</table>

    kable(mhyp)  

<table>
<thead>
<tr>
<th style="text-align:left;">
GO
</th>
<th style="text-align:left;">
male\_hypothalamus\_inc.d9\_inc.d17
</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left;">
candidategenes
</td>
<td style="text-align:left;">
AR
</td>
</tr>
<tr>
<td style="text-align:left;">
cellcycleprocess
</td>
<td style="text-align:left;">
CENPE, CHAMP1, CLASP2, RASSF1, RTTN
</td>
</tr>
<tr>
<td style="text-align:left;">
endocrineprocess
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
lactation
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
matingbehavior
</td>
<td style="text-align:left;">
AR
</td>
</tr>
<tr>
<td style="text-align:left;">
parentalbehavior
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
reproduction
</td>
<td style="text-align:left;">
AR, CLASP2, METTL14, SFRP2, SYNE1
</td>
</tr>
</tbody>
</table>

    kable(fpit)

<table>
<thead>
<tr>
<th style="text-align:left;">
GO
</th>
<th style="text-align:left;">
female\_pituitary\_bldg\_lay
</th>
<th style="text-align:left;">
female\_pituitary\_lay\_inc.d3
</th>
<th style="text-align:left;">
female\_pituitary\_inc.d9\_inc.d17
</th>
<th style="text-align:left;">
female\_pituitary\_inc.d17\_hatch
</th>
<th style="text-align:left;">
female\_pituitary\_hatch\_n5
</th>
<th style="text-align:left;">
female\_pituitary\_n5\_n9
</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left;">
candidategenes
</td>
<td style="text-align:left;">
ESR1, GNRHR
</td>
<td style="text-align:left;">
ESR1, GNRHR
</td>
<td style="text-align:left;">
LBH, PRL
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
AVPR2, LBH, PRL
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
cellcycleprocess
</td>
<td style="text-align:left;">
BRINP2, BTC, CDKN1A, EML1, ESR1, MTBP, NABP1, PDXP, PKIA, RND2, ZWILCH
</td>
<td style="text-align:left;">
AK1, ARF1, BTC, C2CD3, CHMP6, CUL9, EIF4G1, ESR1, HAUS2, ING4, MAP2K1,
NABP1, PTEN, RGCC, RPL24, RPS27L, RPS3, SPIRE1, TRIM37
</td>
<td style="text-align:left;">
ACTR2, ACTR3, ACVR1B, ADAM17, ANLN, APBB1, APP, ARF1, ASPM, ATAD5, ATM,
ATRX, AURKA, AXIN2, AZIN1, BCCIP, BCL2L1, BIRC5, BRCA1, BRIP1, BUB1,
BUB1B, BUB3, CCNA2, CCNB3, CCNE2, CDC20, CDC25A, CDC45, CDC6, CDC7,
CDK1, CDK5R1, CDK5RAP3, CDKN3, CDT1, CENPA, CENPE, CENPF, CENPK, CENPW,
CEP55, CETN1, CETN3, CGRRF1, CHEK1, CKAP2, CKS1B, CKS2, CLSPN, CNTLN,
CRADD, CTDSP1, DCLRE1B, DCTN1, DGKZ, DIS3L2, DNA2, DONSON, DSCC1, DSTN,
DTL, DYNC1LI1, E2F1, E2F4, E2F7, ECT2, EIF2AK4, EIF4E, EME1, EPS8,
ESCO2, ESPL1, FAM107A, FAM83D, FANCD2, FBXO5, FBXW5, FOXM1, GEN1, GINS1,
HAUS8, ID4, ILK, INCENP, KIF11, KIF18A, KIF18B, KIF20A, KIF23, KLHDC8B,
KLHL18, KLHL21, KNSTRN, KNTC1, LLGL2, LMNB1, MAD1L1, MAD2L1, MAD2L1BP,
MAP4, MASTL, MCM2, MCM5, MCM6, MDM4, MEPCE, MIS12, MITD1, MKI67, MND1,
MSH2, MSTO1, MYBL1, MYC, NABP1, NACC2, NASP, NCAPD3, NCAPG, NCAPH,
NCAPH2, NDC1, NDC80, NDE1, NEK2, NFE2L1, NOTCH2, NPM1, NSUN2, NUF2,
NUMA1, NUSAP1, ORC1, PCID2, PCNA, PHF13, PHGDH, PIBF1, PLCB1, PLK1,
PLK4, POGZ, POLA1, PRC1, PRCC, PSMG2, PTTG1, RACGAP1, RAD21, RAD54B,
RAN, RANBP1, RASSF1, RB1, RBBP8, RBM14, RGCC, RTTN, SFPQ, SGSM3,
SLC16A1, SMC2, SMC3, SMC4, SMPD3, SNX33, SOX2, SPC25, SPIRE2, SPRY1,
STMN1, SYDE1, TACC3, TBCE, TCF3, TFDP1, TJP3, TMOD3, TOP2A, TPX2,
TRIP13, TTK, UBE2C, UHRF2, UIMC1, UPF1, USP19, VCP, WEE1, WEE2, ZBTB17,
ZWILCH
</td>
<td style="text-align:left;">
TTK
</td>
<td style="text-align:left;">
ACTR1A, ACTR2, ALKBH4, ALMS1, ANLN, APP, ARF1, AURKA, AZIN1, BCCIP,
BECN1, BIN1, BIRC5, BRCA1, BUB1, BUB1B, C2CD3, CCNA2, CCNB3, CDC20,
CDC25A, CDC45, CDK1, CDK2, CDK5RAP3, CDKN3, CDT1, CENPK, CETN1, CGRRF1,
CHMP4B, CKAP2, CKS1B, CKS2, DDIT3, DGKZ, DRG1, DSTN, E2F1, ECT2, EIF4E,
EME1, EPS8, FAP, INCENP, KIF11, KIF13A, KIF20A, KNSTRN, KNTC1, LMNB1,
MAD2L1, MCM2, MCM5, MITD1, MKI67, MND1, MTBP, MYBBP1A, MYBL1, MYC,
NABP1, NASP, NCAPH, NCOR1, NDC80, NSUN2, NUF2, ORC3, PAXIP1, PCNA,
PDS5A, PKD1, PLK1, PLK3, PTTG1, RACGAP1, RAD1, RALBP1, RAN, RANBP1,
RASSF1, RHOA, RPA2, SGSM3, SLC16A1, SPC25, STMN1, TACC3, TCF3, TCF7L2,
TMOD3, UBE2C, VCP, ZWILCH
</td>
<td style="text-align:left;">
FAP, RACGAP1
</td>
</tr>
<tr>
<td style="text-align:left;">
endocrineprocess
</td>
<td style="text-align:left;">
AGT, BMP6, CGA, GAL
</td>
<td style="text-align:left;">
CGA, RAB11FIP5
</td>
<td style="text-align:left;">
ATP6AP2, CRY1, SMAD4, TMF1
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
ATP6AP2, AVPR2, CRY2, FOXL2, RHOA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
lactation
</td>
<td style="text-align:left;">
XBP1
</td>
<td style="text-align:left;">
XBP1
</td>
<td style="text-align:left;">
EIF2AK3, PRL, SOCS2, STAT5B, XBP1
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
EIF2AK3, PRL, XBP1
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
matingbehavior
</td>
<td style="text-align:left;">
TH
</td>
<td style="text-align:left;">
PTEN
</td>
<td style="text-align:left;">
APP, HDAC4, NCOA1
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
APP, NCOA1
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
parentalbehavior
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
PTEN
</td>
<td style="text-align:left;">
PRL
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
PRL
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
reproduction
</td>
<td style="text-align:left;">
ACSBG2, AGT, ARID4B, BMP6, CEBPA, CGA, DHCR24, ESR1, GATA2, NOS2,
PHLDA2, SEBOX, SERPINB5, SP3, TH, TXNRD3
</td>
<td style="text-align:left;">
AFF4, BIRC2, CGA, ESR1, GAS8, GMCL1, MAP2K1, MDK, PACRG, PARK7, PRDX4,
PTEN, S100A11, SOD1, SP3, SPIRE1, TAF4, VASH2, ZBTB16, ZMIZ1
</td>
<td style="text-align:left;">
ACSBG2, ACTR2, ACTR3, ACVR1B, ADA, AFF4, AKAP9, ANTXR1, APP, ARID1A,
ARID4B, ARID5B, ARNT, ASH1L, ASPM, ATAT1, ATM, ATP8B3, ATR, ATRX, AURKA,
BBS1, BCL2L1, BRIP1, BUB1, BUB1B, BUB3, CACNA1H, CADM1, CALCA, CBS,
CCNB3, CCNE2, CCT2, CCT4, CDC20, CDC25A, CDH1, CKS2, CLGN, CLIC4, CREM,
CXCL12, DLD, DNAJB6, DNMT3A, E2F7, EIF2S2, EIF4H, EME1, EPAS1, ESPL1,
EXO1, FANCC, FANCD2, FANCL, FBXO5, FKBP4, GATA2, GGT5, GLI2, GOLGA3,
GORASP2, GPX4, HDAC4, HEY1, HMGB2, HNF4A, HOOK1, ID4, ITGA3, ITGB4,
JAG2, KATNAL1, KIF18A, KLHDC3, LAMB1, LGR4, LGR5, LLGL2, MASTL, MCM8,
MKI67, MND1, MSH2, MYBL1, NCAPD3, NCAPH, NCAPH2, NCOA1, NDC1, NEK2, NOG,
NOTCH2, NPHP4, NSUN2, NUF2, PAFAH1B2, PGM3, PLCB1, PLCD1, PLK1, PLK4,
PRDX3, PRDX4, PRL, PTGDS, PTTG1, PYGO2, RACGAP1, RAD21, RAD54B, RBBP8,
RBM15, RFX2, RHOBTB3, RPA1, RXRA, SCMH1, SH3PXD2B, SMAD4, SMC2, SMC3,
SMC4, SOD1, SOX2, SP1, SPATA5, SPIRE2, ST14, STAT5B, STXBP1, SYDE1,
TCP1, TFEB, THBD, TMF1, TNK2, TOP2A, TRIP13, TSNAX, TTK, UBE2A, VDAC2,
WDR77, WEE2, WIPF3, WNT3
</td>
<td style="text-align:left;">
TTK
</td>
<td style="text-align:left;">
ABL2, ACSBG2, ACTR2, AFF4, ALMS1, APP, ASH1L, AURKA, BIRC2, BPTF, BUB1,
BUB1B, CACNA1H, CBS, CCNB3, CCT2, CCT4, CD44, CDC20, CDC25A, CDK2,
CEBPA, CKS2, CREM, EIF2S2, EIF4H, EME1, FANCC, FNDC3A, FOXL2, GORASP2,
GPX4, HMGB2, JMJD1C, MKI67, MND1, MYBL1, NCAPH, NCOA1, NCOA6, NSUN2,
NUF2, PARK7, PKD1, PLCD4, PLK1, PRDX3, PRDX4, PRL, PTTG1, RACGAP1, RAD1,
RBM15, RHOBTB3, SCMH1, SNRPA1, SPATA5, TCF7L2, TCP1, UBE2A, VDAC2,
WDR66, WDR77
</td>
<td style="text-align:left;">
AREG, RACGAP1, SEBOX
</td>
</tr>
</tbody>
</table>

    kable(mpit)

<table>
<thead>
<tr>
<th style="text-align:left;">
GO
</th>
<th style="text-align:left;">
male\_pituitary\_inc.d9\_inc.d17
</th>
<th style="text-align:left;">
male\_pituitary\_inc.d17\_hatch
</th>
<th style="text-align:left;">
male\_pituitary\_hatch\_n5
</th>
<th style="text-align:left;">
male\_pituitary\_n5\_n9
</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left;">
candidategenes
</td>
<td style="text-align:left;">
LBH, PRL, VIP
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
cellcycleprocess
</td>
<td style="text-align:left;">
ANLN, ASPM, AURKA, BIRC5, BRCA1, BTC, BUB1, BUB1B, CCNA2, CCNB3, CCNE2,
CDC20, CDC45, CDC6, CDC7, CDK1, CDK5R1, CDT1, CENPA, CENPE, CENPF,
CENPK, CEP55, CKAP2, CKS1B, CKS2, CLSPN, DGKZ, DTL, E2F1, E2F7, ECT2,
EME1, ESCO2, FANCD2, FBXO5, FOXM1, GJC2, HEPACAM, INCENP, KIF11, KIF18B,
KIF20A, KNSTRN, KNTC1, LMNB1, MAD2L1, MASTL, MCM2, MIS12, MKI67, MSH6,
MYBL1, MYC, NASP, NCAPG, NCAPH, NDC80, NEK2, NUF2, ORC1, PCNA, PHGDH,
PLK1, PLK3, PRC1, PTTG1, RACGAP1, SFPQ, SLC16A1, SMC2, SMPD3, SPC25,
SPRY1, STMN1, TACC3, TMOD3, TOP2A, TPPP, UBE2C, UHRF2
</td>
<td style="text-align:left;">
CETN1, GJC2, NABP1
</td>
<td style="text-align:left;">
ADAMTS1, CKS2, EFHC1, NABP1, PCNA, RAN, TACC3
</td>
<td style="text-align:left;">
ANLN, BRCA1, BUB1, CCNB3, CDC20, CDC45, CDK1, CENPF, CKAP2, CKS1B, E2F1,
ESCO2, KNSTRN, KNTC1, MCM2, MKI67, MYC, NDC80, NUF2, PLK2, RACGAP1,
SPRY1, UBE2C
</td>
</tr>
<tr>
<td style="text-align:left;">
endocrineprocess
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
lactation
</td>
<td style="text-align:left;">
PRL
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
CDO1
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
matingbehavior
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
parentalbehavior
</td>
<td style="text-align:left;">
PRL
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
reproduction
</td>
<td style="text-align:left;">
ARID4B, ASPM, AURKA, BUB1, BUB1B, CCNB3, CCNE2, CDC20, CKS2, CLDN11,
CREM, E2F7, EME1, ETNK2, FANCC, FANCD2, FBXO5, FEM1B, FETUB, GOLGA3,
HMGB2, HOOK1, MASTL, MCM8, MKI67, MSH6, MYBL1, NCAPH, NEK2, NUF2, PLK1,
PRL, PTTG1, PYGO2, RACGAP1, RHOBTB3, SMC2, TOP2A, VIP
</td>
<td style="text-align:left;">
AHSG, ETNK2, SHH, SOX8
</td>
<td style="text-align:left;">
ADAMTS1, CKS2, EIF2S2, PRDX3, TCP1, UBE2A, VDAC2, WDR77, ZBTB16
</td>
<td style="text-align:left;">
BUB1, CCNB3, CDC20, MKI67, NUF2, PHLDA2, RACGAP1
</td>
</tr>
</tbody>
</table>

    kable(fgon) 

<table>
<thead>
<tr>
<th style="text-align:left;">
GO
</th>
<th style="text-align:left;">
female\_gonad\_bldg\_lay
</th>
<th style="text-align:left;">
female\_gonad\_lay\_inc.d3
</th>
<th style="text-align:left;">
female\_gonad\_inc.d3\_inc.d9
</th>
<th style="text-align:left;">
female\_gonad\_inc.d9\_inc.d17
</th>
<th style="text-align:left;">
female\_gonad\_n5\_n9
</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left;">
candidategenes
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
AVPR1A, EGR1, FOS, PRLR
</td>
<td style="text-align:left;">
AVPR1A
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
cellcycleprocess
</td>
<td style="text-align:left;">
AKT1, ANAPC15, ANP32B, CCNG2, CDKN1B, CENPA, CENPF, CENPK, CHMP1A,
CKS1B, CNTLN, DCTN6, DGKZ, DONSON, FHL1, GPNMB, IGF1, INCENP, KIF18B,
MKI67, NPM1, NSFL1C, PEBP1, PKD2, PKHD1, POU4F1, PPM1G, RAN, RBBP8,
RCC1, RPA2, RPL24, RPS3, SIRT2, SMC4, SOX2, STARD9, SUN1, TACC2, TFDP1,
TUBGCP6, UBE2S, UVRAG, WASL, ZFP36L1, ZWILCH
</td>
<td style="text-align:left;">
ACTR10, ACTR3, AK1, ALMS1, ANAPC15, ANK3, ANKLE2, ANP32B, ARF1, BABAM1,
BBS4, BCCIP, BIRC5, BIRC6, BRCC3, C2CD3, CACUL1, CD28, CDC23, CDC26,
CDC27, CEP250, CHMP1A, CHMP3, CHMP4C, CHMP6, CKS1B, CKS2, CRADD, CROCC,
CTC1, DAB2IP, DACT1, DCTN6, DRG1, DYRK3, EGF, EIF4E, EPS8, ERN1,
FAM107A, FANCM, FBXL18, FGFR3, GJA1, GLI1, HDAC8, HTT, IGF1R, JADE1,
KIF13A, KIF2A, KLHL13, LIMK2, LLGL2, LRP5, MCPH1, MED1, MIIP, MISP,
MYH10, MYO19, NABP2, NACC2, NEK6, NPAT, NPM1, NSMCE2, NUBP1, ODF2,
PAFAH1B1, PCID2, PCNT, PDE3A, PDPN, PEBP1, PHGDH, PIDD1, PKD1, PLCB1,
PLK2, PLRG1, PRKDC, PSMC3IP, RAD51B, RAN, RBL2, RCC2, RHOA, RHOU, RND3,
RNF212, ROCK2, RPL23, RPL24, RPRM, RPS3, RPS6, RPTOR, RRP8, SLX4, SOX2,
SPIRE1, SPTBN1, SSTR5, STMN1, SYF2, TAF10, TAF2, TFDP1, THBS1, TOM1L1,
TPPP, TRRAP, TUBB1, TXLNG, UVRAG, WNT4, XRCC2, YTHDF2, YWHAH
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
ANXA1, ARL3, BBS4, CCNJ, CDC14A, CEP126, CROCC, DYRK3, EFHC1, ID4,
INSM1, MAPK15, MLH1, MLXIPL, NEK10, NEK11, ODF2, PARD6B, PCNT, RBM38,
SOX9, TMEM67, UNC119
</td>
<td style="text-align:left;">
CETN1, CTDSPL, CYP26B1, EFHC1, FAM107A, MLF1, ODF2, ZFP36L2
</td>
</tr>
<tr>
<td style="text-align:left;">
endocrineprocess
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
AVPR1A, C1QTNF1, CRY2, F2R, GJA1, RHOA
</td>
<td style="text-align:left;">
AVPR1A
</td>
<td style="text-align:left;">
BMP6, DRD5
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
lactation
</td>
<td style="text-align:left;">
NME1
</td>
<td style="text-align:left;">
ATP2B2, CAV1, GJA1, HIF1A, MED1, PRLR, XBP1, ZBTB7B
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
ZBTB7B
</td>
</tr>
<tr>
<td style="text-align:left;">
matingbehavior
</td>
<td style="text-align:left;">
APLP2, CYP11A1
</td>
<td style="text-align:left;">
AVPR1A, NCOA2
</td>
<td style="text-align:left;">
AVPR1A
</td>
<td style="text-align:left;">
DRD5
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
parentalbehavior
</td>
<td style="text-align:left;">
CREBRF
</td>
<td style="text-align:left;">
AVPR1A, CREBRF, GNAQ, NR3C1
</td>
<td style="text-align:left;">
AVPR1A
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
reproduction
</td>
<td style="text-align:left;">
ADA, ADRA2C, AKT1, ALKBH1, ANXA5, APLP2, CBS, CCDC42, CCT6A, CDKN1B,
CREBRF, CREM, CYP11A1, DACH2, DHCR24, DNAJA1, DPCD, EDNRB, EPAS1, FKBP4,
FOXC1, GAB1, HMGB2, HTRA1, IGF1, IMMP2L, LGR5, MDK, MKI67, PARK7, PEBP1,
PKD2, PLTP, PRDX3, PSAP, RBBP8, RNF151, SEMA3A, SFRP2, SIRT2, SMC4,
SOX17, SOX2, SRD5A1, SUN1, TDRD7, TSGA10, TUBGCP6, VDAC2, VWF, ZFP36L1
</td>
<td style="text-align:left;">
ABAT, ACOX1, ACTR3, ADM, ADRM1, AFF4, ALKBH1, ALMS1, ANXA5, APELA,
ARNTL, ASH1L, AVPR1A, BBS4, BIRC6, BMPR2, C1QBP, CBS, CCT2, CCT4, CCT5,
CCT6A, CCT8, CD9, CFTR, CKS2, CLOCK, CREBRF, CREM, CXCR4, DNAH5, EDNRB,
EGR1, EIF2S2, EIF4H, ENO4, ENPP2, EPAS1, ETV6, FANCM, FGF7, FOXC1,
FOXO3, GAMT, GDF7, GGT1, GGT5, GJA1, GLI1, GNAQ, GOLGA3, GPX4, GREB1L,
HECTD1, HIF1A, HMGB2, HOOK1, HPGDS, HTT, IGF1R, IMMP2L, ITGB4, JMJD1C,
LHFPL2, LHX1, LIMK2, LLGL2, LRGUK, MDFI, MDK, MED1, MMP2, MMP9, MTOR,
NCOA2, NCOA3, NCOA6, NDP, NR0B1, NR2C2, NR3C1, ODF2, PAFAH1B1, PARK7,
PCSK5, PDE3A, PDE5A, PDGFRA, PEBP1, PGM3, PKD1, PLAG1, PLAU, PLCB1,
PLD6, PLG, PPARG, PRDX3, PRDX4, PRKDC, PRLR, PSMC3IP, PSME4, PTGS2, PTN,
RAI14, RBPJ, RLN1, RNF212, RPS6, SEMA3A, SETX, SIX4, SLX4, SMURF2,
SNRPA1, SNU13, SOD1, SOS1, SOX2, SPATA18, SPATA6, SPIRE1, STXBP1, SYNE1,
TBPL1, TCF21, TIPARP, TSNAX, TTLL5, TTPA, USP42, VDAC2, WNT4, YTHDF2,
ZFPM2, ZMIZ1, ZP3
</td>
<td style="text-align:left;">
AVPR1A
</td>
<td style="text-align:left;">
ACOX1, AK7, ANXA1, ARMC2, BBS1, BBS4, BMP6, CADM1, CCDC33, CCDC39,
CCDC40, CEBPA, CFAP43, CFAP44, CFTR, DNAH5, DRD5, EFCAB1, ENO4, FRS2,
ID4, IFT88, LRGUK, MAPK15, MLH1, NPHP4, NSUN7, ODF2, PACRG, PIH1D3,
RSPH1, SOX17, SOX9, SPATA6, SPEF2, TEKT2, TEKT4, TPPP3, TSNAXIP1, TTC26,
WDR66
</td>
<td style="text-align:left;">
ARMC2, BBS1, CIB1, CYP26B1, DPCD, EFCAB1, GAS8, GJB5, KLHL10, LRGUK,
ODF2, PACRG, PGM3, PIH1D3, PKDREJ, PLCZ1, PPARD, PTK2B, ROPN1L, RSPH1,
SBF1, SEBOX, SPAG8, SPATA18, SPINT1, TEKT2, TEKT4, TIPARP, TPPP2, TPPP3
</td>
</tr>
</tbody>
</table>

    kable(mgon) 

<table>
<thead>
<tr>
<th style="text-align:left;">
GO
</th>
<th style="text-align:left;">
male\_gonad\_bldg\_lay
</th>
<th style="text-align:left;">
male\_gonad\_lay\_inc.d3
</th>
</tr>
</thead>
<tbody>
<tr>
<td style="text-align:left;">
candidategenes
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
cellcycleprocess
</td>
<td style="text-align:left;">
CDKN3, CEP76, IK, IST1, KDM8, PDS5A, SYF2
</td>
<td style="text-align:left;">
ATP2B4, NPAT, PRKDC, PTTG1, SBDS, SH3GLB1, SOX9
</td>
</tr>
<tr>
<td style="text-align:left;">
endocrineprocess
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
ACE2
</td>
</tr>
<tr>
<td style="text-align:left;">
lactation
</td>
<td style="text-align:left;">
EIF2AK3
</td>
<td style="text-align:left;">
HIF1A
</td>
</tr>
<tr>
<td style="text-align:left;">
matingbehavior
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
parentalbehavior
</td>
<td style="text-align:left;">
NA
</td>
<td style="text-align:left;">
NA
</td>
</tr>
<tr>
<td style="text-align:left;">
reproduction
</td>
<td style="text-align:left;">
BOLL, NR2C2, ROPN1L, SP3
</td>
<td style="text-align:left;">
ACOX1, ANXA5, ATP2B4, FOXC1, HIF1A, KIT, PRKDC, PTTG1, SOX9
</td>
</tr>
</tbody>
</table>

    write.csv(GO_DEGs, "../results/12_GO_DEGs.csv")
    #write.csv(hyp, "../results/12_GO_DEGs_hyp.csv")
    #write.csv(pit, "../results/12_GO_DEGs_pit.csv")
    #write.csv(gon, "../results/12_GO_DEGs_gon.csv")
