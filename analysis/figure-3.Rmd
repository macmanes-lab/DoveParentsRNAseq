...
```{r gon}
# pca
charpca <- subsetmakepca("gonads", charlevels, sexlevels)	
charfviz <- makefvizdf("gonads", charlevels, sexlevels)
...
b <- plotfriz(charfviz) + labs(subtitle = "gonad") +
  #ylim(-250000,500000) + xlim(-250000,500000) + 
  theme(axis.text = element_blank())
...
ggplot(charpca, aes(x = PC1, y = PC2, shape = tissue, color = treatment )) +	
    geom_point(size = 1)  +	
  stat_ellipse( ) +
    theme_B3() +
  scale_color_manual(values = colorschar)

ggplot(charpca, aes(x = PC1, y = PC3, shape = tissue, color = treatment )) +	
    geom_point(size = 1)  +	
  stat_ellipse( ) +
    theme_B3() +
  scale_color_manual(values = colorschar) +
