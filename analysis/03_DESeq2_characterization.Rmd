...
musicgenes5 <- function(mygroup, mysusbtitle){
  myplot <- candidataCountsTcol2 %>%
  filter(sextissue %in% c(mygroup),
         gene == "GAL") %>%
    ggplot(aes(x = treatment, y = mean)) +
...
a <- musicgenes5("female_hypothalamus", "female hypothalamus")
b <- musicgenes5("male_hypothalamus", "male hypothalamus")

c <- plot_grid(a,b)
c

pdf("../figures/characterization/musicalgenes-6.pdf", width = 6, height = 3)
plot(c)
dev.off()
