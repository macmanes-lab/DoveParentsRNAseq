...
parentalcareGO <- c("AVP", "AVPR1A", "BRINP1", "CREBRF", "DBH", "DRD1",  
                    "GNAQ", "HUBB", "KALRN", "MBD2","NPAS1", "NPAS3",
                     "NR3C1", "OPRK1", "OXT", "OXTR", "PRL", "PTEN")

# can't find NOS1 or OXTR or FOX1B or HTR5A
```
...
hypvsd <- getcandidatevsd(curleychampagnegenes, "hypothalamus", sexlevels)
pitvsd <- getcandidatevsd(curleychampagnegenes, "pituitary", sexlevels)
gonvsd <- getcandidatevsd(curleychampagnegenes, "gonad", sexlevels)
head(hypvsd)
tail(gonvsd)
...
candidateDEGS <- read_csv("../../musicalgenes/data/allDEG.csv") %>%
  select(-X1) %>%
  filter(gene %in% curleychampagnegenes)
head(candidateDEGS)
...
candidateDEGtable <- candidateDEGS %>%
  ...
  select(sex, tissue, bldg_lay, lay_inc.d3, inc.d3_inc.d9,  
         inc.d9_inc.d17,  hatch_n5, control_bldg)
candidateDEGtable
```
