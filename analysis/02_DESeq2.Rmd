...
bottom <- plotbarvolcano(PRLdegs, c("c","d"), "prolactin expression")
...
ggplot(sexPRLdesg, aes(x = direction.x, fill = direction.y)) +
  geom_bar(position = "dodge") +
  scale_fill_manual(values = allcolors, na.value= "#bdbdbd",
                    labels= c("low PRL", "hi PRL", "NS"),
                    name = "PRL DEGs") +
  scale_x_discrete(breaks = c("female", "male", NA),
                   labels = c("female", "male", "NS"),
                   name = "Sex DEGs") 

interaction <- sexPRLdesg %>% filter(direction.x %in% c("male", "female") & 
                                       direction.y %in% c("hi", "lo"))
interaction

sexonly <- sexPRLdesg %>% filter(direction.x %in% c("male", "female") & 
                                       !direction.y %in% c("hi", "lo") )
sexonly
PRLonly <- sexPRLdesg %>% filter(!direction.x %in% c("male", "female")& 
                                       direction.y %in% c("hi", "lo"))
PRLonly
