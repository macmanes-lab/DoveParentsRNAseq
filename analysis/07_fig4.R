...
a1 <- plotcandidatechar(gonvsdf, "PRL") + labs(subtitle = "gonads")
a2 <- plotcandidatechar(gonvsdm, "PRL") + labs(y = NULL, x = "") + labs(subtitle = " ")

b1 <- plot.volcano("gonads", sexlevels,  "control_inc.d17") + 
  facet_wrap(~sex) 
b2 <- plot.volcano("gonads", sexlevels,  "inc.d9_inc.d17") + 
  facet_wrap(~sex) 
...
f1 <- plotremoval(gonvsdf, "PRL")+ labs(y = NULL) + labs(subtitle = " ")
f2 <- plotremoval(gonvsdm, "PRL") + labs(y = NULL, x = "")+ labs(subtitle = " ")

g1 <- plotreplacement(gonvsdf, "PRL") + labs(y = NULL)+ labs(subtitle = " ")
g2 <- plotreplacement(gonvsdm, "PRL") + labs(y = NULL, x = "")+ labs(subtitle = " ")


#h <- makebargraphv3(DEGremove, "gonads", "No. of DEGs\n (-)decreased  increased (+)", comparisonlablelsremoval, comparisonlevelsremoval) +
  labs(x = "Offspring removal versus temporal control")

i <- makebargraphv3(DEGreplace, "gonads", NULL, comparisonlablelssreplace, comparisonlevelsreplace) +
...
fig4 <- plot_grid(ab,c,de, fghi, nrow = 4, 
