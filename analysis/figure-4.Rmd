...
DEGs <- read_csv("../results/DEGs-char.csv")  %>%
  dplyr::filter(comparison != "control_bldg") %>%
  dplyr::distinct(gene)  %>% arrange(gene) %>%
  filter(!grepl("LOC", gene)) #%>% 
  #pull(gene)


HUGO <- read_table("../metadata/HUGOGeneNomenclature.txt")
names(HUGO)[1] <- "all"
HUGO$gene <- sapply(strsplit(HUGO$all,'\t'), "[", 3)
HUGO$name <- sapply(strsplit(HUGO$all,'\t'), "[", 4)
HUGO <- HUGO %>%distinct(gene,name)

DEGsHUGO <- inner_join(DEGs, HUGO, by = "gene") %>% pull(gene)


parentalbehavior <- read_table("../results/goterms/parentalbehavior.txt")
...
returnvsds <- function(whichgenes){
  ...
  df  <- allvsd %>%
    dplyr::filter(!grepl("control", samples)) %>%
    dplyr::filter(gene %in% whichgenes) %>%
...
#DEGvsd <- returnvsds(DEGsHUGO) too big 1G
parentalvsd <- returnvsds(parentalbehaviorgenes)
```
...
write.csv(parentalvsd, "../../musicalgenes/data/parentalcounts.csv")
