...
getcandidatevsdmanip <- function(whichgenes, whichtissue){
  ...
  candidates  <- allvsd %>%
    ...
    dplyr::mutate(sextissue = sapply(strsplit(file_name, '_vsd.csv'), "[", 1),
                ...
                treatment =  factor(candidates$treatment, levels = alllevels),
                ...
                extint = fct_collapse(treatment, 
                                  ...
                                  chicks = c( "early", "hatch", "extend", "n5"),
                                  loss = c("m.inc.d3", "m.inc.d9", "m.n2", "m.inc.d17", "bldg")),
                manip = factor(manip, levels = c("reference", "removal", "replacement")))
  return(candidates)
} 
  %>%
    dplyr::select(gene, counts, sex, tissue, treatment, day, manip, earlylate, extint, samples)  %>%
...
hypvsd <- getcandidatevsdmanip(c("PRL", "DRD1"), "hypothalamus")
pitvsd <- getcandidatevsdmanip(candidategenes, "pituitary")
gonvsd <- getcandidatevsdmanip(candidategenes, "gonad")
...
plotcandidatetiming <- function(df, whichgene, whichtissue){
  ...
  dfslim <- df %>%
    dplyr::filter(gene  == whichgene, treatment %in% timing) 
  print(head(dfslim))
...
a <- plotcandidatetiming(hypvsd, "DRD1", "hypothalamus")
b <- plotcandidatetiming(pitvsd, "PRL", "pituitary")
c <- plotcandidatetiming(gonvsd, "CRHBP", "gonads")
