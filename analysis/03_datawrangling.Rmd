...
pseudocounts <- as.data.frame(t(pseudocounts))
head(pseudocounts[1:3])

pseudocountsLong <- pseudocounts %>%
  mutate(samples = row.names(.)) %>%
  pivot_longer(-samples, names_to = "gene", values_to = "pseudocounts")
#pseudocountsLong$samples <- row.names(pseudocountsLong)
head(pseudocountsLong)
```
...
head(row.names(pseudocounts) == row.names(colData))
```
