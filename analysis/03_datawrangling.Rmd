...
Note, limmacounts is too big for storage on github :(

```{r}
limmacounts <- read_csv("../results/01_limma.csv")
head(limmacounts[1:3])
limmacounts <- as.data.frame(limmacounts)
row.names(limmacounts) <- limmacounts$X1
limmacounts$X1 <- NULL
# prep count data for all samples
limmacounts <- as.data.frame(t(limmacounts))
head(limmacounts[1:3])

limmacountsLong <- limmacounts %>%
  mutate(samples = row.names(.)) %>%
  pivot_longer(-samples, names_to = "gene", values_to = "limmacounts")
#limmacountsLong$samples <- row.names(limmacountsLong)
head(limmacountsLong)
```
...
head(row.names(limmacounts) == row.names(colData))
```
